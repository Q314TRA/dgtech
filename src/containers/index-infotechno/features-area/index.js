...
const FeaturesArea = (props) => {
    const featuredDataQuery = useStaticQuery(graphql`
        query infotechnoFeatureQueryData {
            landingJson(id: {eq: "landing-featured-content"}) {
                title
                subtitle
            }
    ...
    `);
    const featureSecData = featuredDataQuery.landingJson;
    ...
    return (
        <SectionWrap id="services">
            <Container>
                ...
                <Row>
                    <Col lg={12}>
                        <Heading {...headingStyle}>Crea con nosotros. <Anchor {...linkStyle} path="#contact">Acepta el reto!</Anchor></Heading>
                    </Col>
                </Row>
            </Container>
        </SectionWrap>
